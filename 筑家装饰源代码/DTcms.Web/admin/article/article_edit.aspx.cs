    ...
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (channel_id == 7)
                {
                    this.dlDesignerID.Style.Add("display", "inherit");
                    this.ddlDesignerID.Visible = true;
                    TreeBindForDesigner();//绑定设计师
                }
        ...
        #region 绑定设计师=================================
        private void TreeBindForDesigner()
        {
            BLL.article bll = new BLL.article();
            DataTable dt = bll.GetList(0, " channel_id=10", " [sort_id] ").Tables[0];
            this.ddlDesignerID.Items.Clear();
            this.ddlDesignerID.Items.Add(new ListItem("请选择类别...", ""));
            foreach (DataRow dr in dt.Rows)
            {
                string Id = dr["id"].ToString();
                string Title = dr["title"].ToString().Trim();
                this.ddlDesignerID.Items.Add(new ListItem(Title, Id));
            }
        ...
        private void ShowInfo(int _id)
        {
            ...
            this.ddlDesignerID.SelectedValue = model.designer_id.ToString ();
        ...
        private bool DoEdit(int _id)
        {
            ...
            BLL.article bll = new BLL.article();
            Model.article model = bll.GetModel(_id);
            model.designer_id = int.Parse(this.ddlDesignerID.SelectedValue == "" ? "0" : this.ddlDesignerID.SelectedValue);
            model.channel_id = this.channel_id;
            model.category_id = Utils.StrToInt(ddlCategoryId.SelectedValue, 0);
            model.call_index = txtCallIndex.Text.Trim();
            model.title = txtTitle.Text.Trim();
