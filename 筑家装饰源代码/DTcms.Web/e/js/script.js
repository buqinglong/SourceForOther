$(document).ready(function(){   
   ...
   crgd("module_220","/index.aspx?lanmuid=86&sublanmuid=804","查看更多");//特价游经典游
   crgd("module_221","/index.aspx?lanmuid=85&sublanmuid=813","查看更多");//安心旅游品质游
...
$(window).scroll(function(){                   
	...
	var $banner_hei=$(".banner").height();
	var $scrolltop=$(document).scrollTop();
	var $top_box=$(".top_box");
	var $window_hei=$(window).height();
	$(window).scroll(function() {
		var $after = $(window).scrollTop();
		if ($before>$after && $scrolltop>$banner_hei) {
			$top_box.addClass("top_up");
		}else if($before>$after && $scrolltop<=$banner_hei)
		{
			$top_box.removeClass("top_up");
		}
		if ($before<$after) {
		    var $top_up_in_length=$(".top_up").find(".menu_box.in").length
		    if($top_up_in_length>=1)
		    {
		        $(".menu_box").css({"max-height":($window_hei-70)+"px"});
		    }
			$top_box.removeClass("top_up");
		};
	});
...
function crgd(objname,moreurl,wz)
{
	$("#"+objname).children(".module_box_inner").append('<div class="wc_more"><a class="mk_more" href='+moreurl+'>'+wz+'</a></div>');
}
