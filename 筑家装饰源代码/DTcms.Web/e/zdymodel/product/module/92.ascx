...
<%}%><%conn.Open();%><div class="marquee_pic marquee" id="m_<%=Module_Id%>">
  <div class="inner">
   <ul>
<% 
...
%>
<li class="hx">
<a class="pic" href="<%=Detail_Url(dr)%>" target="<%=Target%>"><img src="<%=dr["titlepic"]%>" border="0"></a>
<a class="title" href="<%=Detail_Url(dr)%>" target="<%=Target%>" title="<%=Server.HtmlEncode(dr["title"].ToString())%>"><%=SubStr(dr["title"].ToString(),Title_Num,true)%></a>
</li>
<%
 }
%>
...
<div class="zybk_an"><a class="prev"><</a><a class="next">></a></div><!--控制按钮-->
</div>
<script type="text/javascript">
marquee("m_<%=Module_Id%>","left",100000);
</script><%conn.Close();%>
