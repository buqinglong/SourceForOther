...
<%}%><%conn.Open();%><ul class="mkmx_dyttw_box">
<% 
string pic,isTop;
...
%>
  <li class="dyttw_twjj">
    <a class="dyttw_pic" href="<%=Detail_Url(dr)%>"><img src="<%=dr["titlepic"].ToString()%>"/></a>
    <div class="dyttw_con">
      <a class="title" href="<%=Detail_Url(dr)%>"><%=dr["title"].ToString()%></a> 
      <div class="con"><%=dr["pa_introduct"].ToString()%></div>
    </div>
  </li>
<%}else{%>
  <li class="dyttw_bt"><a href="<%=Detail_Url(dr)%>"><%=dr["title"].ToString()%></a></li>
<%
}
 }
%>
