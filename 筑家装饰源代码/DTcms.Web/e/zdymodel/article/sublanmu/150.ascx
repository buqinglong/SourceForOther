...
<%if(IsSearch==0){conn.Open();%><ul id="accordion" class="jjyc">
<% 
...
if(i==0){
%>
<li class="yc_com panel">
  <div data-parent="#accordion" data-toggle="collapse" data-target="#cjwt<%=i%>" class="tits current"><%=dr["title"].ToString()%><span class="s1">-</span></div>
  <div id="cjwt<%=i%>" class="coms collapse in"><%=dr["content"].ToString()%></div>
</li>
<%}else{%>
<li class="yc_com panel">
  <div data-parent="#accordion" data-toggle="collapse" data-target="#cjwt<%=i%>" class="tits"><%=dr["title"].ToString()%><span class="s1">+</span></div>
  <div id="cjwt<%=i%>" class="coms collapse"><%=dr["content"].ToString()%></div>
</li>
<%
}
 }
...
<script>
$(function(){
  $(".yc_com").each(function(i){
    $("#cjwt"+i).on("hide.bs.collapse",function(){
      $("#cjwt"+i).siblings(".tits").children(".s1").text("+")
      $("#cjwt"+i).siblings(".tits").removeClass("current")
    });
    $("#cjwt"+i).on("show.bs.collapse",function(){
      $("#cjwt"+i).siblings(".tits").addClass("current")
      $("#cjwt"+i).siblings(".tits").children(".s1").text("-")
    });
  });
});
