...
<%if(IsSearch==0){conn.Open();%><ul class="wzlb_zbrq_box">
<% 
...
%>
  <li>
    <div class="date col-sm-1 col-xs-3">
<span class="date1"><%=((DateTime)dr["thedate"]).ToString("dd")%></span>
<span class="date2"><%=((DateTime)dr["thedate"]).ToString("yyyy-MM")%></span>
    </div>
    <div class="con col-sm-11 col-xs-9">
<a class="title" href="<%=Detail_Url(dr)%>"><%=dr["title"].ToString()%></a>
<div class="introduct"><%=dr["pa_introduct"].ToString()%></div>
    </div>
  </li>
<%
 }
