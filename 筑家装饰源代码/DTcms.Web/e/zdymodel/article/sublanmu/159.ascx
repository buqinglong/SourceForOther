...
<%if(IsSearch==0){conn.Open();%><ul class="tpzs_box">
<% 
...
%>
  <li class="animates4 col-md-3 col-xs-6">
<a class="pic" href="<%=Detail_Url(dr)%>"><img src="<%=dr["titlepic"].ToString()%>"/></a>
<a class="tpzs_title" href="<%=Detail_Url(dr)%>"><%=dr["title"].ToString()%></a>
  </li>
<%
 }
%>
