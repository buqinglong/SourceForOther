...
<%if(IsSearch==0){conn.Open();%><ul class="zyyw_box">
<% 
string istop="0";
...
if(istop=="1"){
%>
  <li class="zyyw_animate col-sm-6 col-xs-12">
<a class="zyyw_pic" href="<%=Detail_Url(dr)%>">
  <img src="<%=dr["titlepic"].ToString()%>"/>
  <div class="zyyw_title" href="<%=Detail_Url(dr)%>"><span><%=dr["title"].ToString()%></span><em></em></div>
</a>
  </li>
<%}else{%>
  <li class="zyyw_animate col-sm-3 col-xs-12">
<a class="zyyw_pic" href="<%=Detail_Url(dr)%>">
  <img src="<%=dr["titlepic"].ToString()%>"/>
  <div class="zyyw_title" href="<%=Detail_Url(dr)%>"><span><%=dr["title"].ToString()%></span><em></em></div>
</a>
