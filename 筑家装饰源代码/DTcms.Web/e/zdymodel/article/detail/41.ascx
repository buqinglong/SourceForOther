...
<div class="tpz"><iframe src="/e/images/picsview/pics.aspx?num=<%=News_Pic%>" frameborder=0 align=middle marginheight=0 marginwidth=0 scrolling=no></iframe></div>
<%}//组结束%>-->
<div class="content" id="Content">
<%if(video!=""){%>
<video id="example_video_1" class="video-js vjs-default-skin" controls preload="none"  width="100%" height="300"  poster="<%=dr["titlepic"].ToString()%>" data-setup="{}">
    <source src="<%=dr["pa_video"].ToString()%>" type="video/mp4" />
    <source src="<%=dr["pa_video"].ToString()%>" type="video/webm" />
    <source src="<%=dr["pa_video"].ToString()%>" type="video/ogg" />
您的浏览器不支持Video标签。
</video>
<%}%>
<%=dr["content"].ToString()%>
